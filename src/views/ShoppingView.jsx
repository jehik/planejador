...
const ShoppingView = () => {
    const { userData, addShoppingItem, toggleShoppingItem, removeShoppingItem } = useAppStore();
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
                {items.length === 0 ? (
                    <div style={{ textAlign: 'center', padding: '60px 24px', backgroundColor: 'var(--surface-color)', borderRadius: '32px', border: '1px dashed var(--border-color)' }}>
                        <div style={{ width: '64px', height: '64px', borderRadius: '20px', backgroundColor: 'rgba(0,0,0,0.03)', display: 'flex', alignItems: 'center', justifyContent: 'center', margin: '0 auto 20px auto', color: 'var(--text-tertiary)' }}>
                            <ShoppingCart size={32} />
                        </div>
                        <h3 style={{ fontSize: '1.1rem', fontWeight: '800', marginBottom: '8px' }}>Lista Vazia</h3>
                        <p style={{ color: 'var(--text-tertiary)', fontSize: '0.9rem' }}>Nenhum item adicionado à lista de compras ainda.</p>
                    </div>
                ) : (
                    items.map(item => (
                        <div key={item.id} className="card fade-in" style={{ padding: '16px 20px', display: 'flex', alignItems: 'center', gap: '16px', border: '1px solid var(--border-color)', opacity: item.completed ? 0.6 : 1 }}>
                            ...
                            <div style={{ flex: 1 }}>
                                ...
                                <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                                    <span style={{ fontSize: '0.65rem', fontWeight: '800', padding: '2px 8px', borderRadius: '6px', backgroundColor: 'rgba(0,0,0,0.05)', color: 'var(--text-tertiary)', textTransform: 'uppercase' }}>{item.category}</span>
                                </div>
                            </div>
